...
abstract class OAuth2
{
   ...
   * @todo http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-5.1.3
  ...
  public function grantAccessToken()
  {
    ...
    if (!$input["scope"]) $input["scope"] = NULL;
  ...
  public function finishClientAuthorization($is_authorized, $params = array())
  {
    ...
    if ($state !== NULL) $result["query"]["state"] = $state;
  ...
  protected function buildUri($uri, $params)
  {
    ...
    {
      if (isset($parse_url[$k])) $parse_url[$k] .= "&" . http_build_query($v);
      else $parse_url[$k] = http_build_query($v);
    }
  ...
  protected function errorDoRedirectUriCallback($redirect_uri, $error, $error_description = NULL, $error_uri = NULL, $state = NULL)
  {
    ...
    if ($state) $result["query"]["state"] = $state;
   ...
   * @ingroup oauth2_error
   */
  protected function errorJsonResponse($http_status_code, $error, $error_description = NULL, $error_uri = NULL)
  {
    $result['error'] = $error;
